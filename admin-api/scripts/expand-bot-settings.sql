-- Migration: Expand bot_settings table with comprehensive admin settings
-- Purpose: Add all necessary settings for withdrawal management, limits, security, and infrastructure
...
-- Add comments
COMMENT ON COLUMN bot_settings.withdrawal_wallet_address IS 'Admin wallet address for fee withdrawals';
COMMENT ON COLUMN bot_settings.emergency_stop IS 'Immediately halt all trading operations';
COMMENT ON COLUMN bot_settings.auto_collect_fees IS 'Automatically collect fees to withdrawal wallet';
COMMENT ON COLUMN bot_settings.enable_mev_protection IS 'Enable MEV protection for all trades';
COMMENT ON COLUMN bot_settings.solana_rpc_endpoint IS 'Primary Solana RPC endpoint';
COMMENT ON COLUMN bot_settings.max_wallets_per_user IS 'Maximum number of wallets a user can create';
COMMENT ON COLUMN bot_settings.daily_trade_limit_per_user IS 'Maximum SOL a user can trade per day';
